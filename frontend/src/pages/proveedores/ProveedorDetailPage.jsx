...
const ProveedorDetailPage = () => {
  ...
  const cargar = async () => {
    try {
      const { data } = await axios.get(`/proveedores/${id}/historial`);
      setProveedor(data);
    } catch {
  ...
  const handleCrearContacto = async (form) => {
    setSaving(true);
    try {
      await axios.post(`/proveedores/${id}/contactos`, form);
  ...
  const handleEditarContacto = async (form) => {
    setSaving(true);
    try {
      await axios.put(`/proveedores/contactos/${editContacto.id}`, form);
    ...
    <div>
      <div className="flex items-start gap-3 mb-6">
        ...
        <div className="flex-1">
          <h2 className="text-2xl font-bold text-gray-800">{proveedor.razon_social}</h2>
          <div className="flex items-center gap-3 mt-1 text-sm text-gray-400 flex-wrap">
            {proveedor.ruc && <span>RUC: <strong className="text-gray-600 font-mono">{proveedor.ruc}</strong></span>}
            {proveedor.telefono && <span>· {proveedor.telefono}</span>}
            {proveedor.web && <span>· {proveedor.web}</span>}
          </div>
          {proveedor.descripcion && (
            <p className="mt-2 text-sm text-gray-600 max-w-3xl">{proveedor.descripcion}</p>
          )}
        </div>
        <button
      ...
      <div className="bg-white rounded-2xl overflow-hidden" style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h3 className="font-semibold text-gray-800">Contactos del proveedor</h3>
        </div>
